...
> **AI 辅助二筛**、**智能约面排期**，并把状态**实时同步**到可视化看板。
...
- **流程自动推进** — Agent 自动完成机械阶段：简历收集 → HR 初筛 → 拉群协作 → 部门二筛（含自动汇总腾讯文档）
- **AI 辅助二筛** — 按用人部门需求**生成专业化提示词**，AI 辅助二筛并给出**置信度评价**：
  - **≥ 80% 直接过**（自动推进群面名单）
...
| 阶段 | 谁处理 | 说明 |
|------|--------|------|
| 简历收集 → HR初筛 → 拉群协作 → 部门二筛 | **Agent 自动** | 收集/汇总/建档/机械推进 |
| 部门二筛（AI 二筛） | **HR 判断闸口** | HR 在「AI 二筛」面板生成提示词、运行二筛、确认置信度与决策 |
...
3. **AI 二筛** — 待二筛候选人列表 + 二筛工作台（部门需求 / 生成提示词 / 运行二筛 / 结果）+ 二筛记录
4. **约面排期** — 面试官可约时间编辑 + 待约面候选人 + 面试排期表
5. **实时监控** — 实时事件流 + Agent 行为日志
...
| `/api/screening-records` | GET | 二筛记录（含置信度与决策） |
...
- **约面排期**：在「约面排期」页编辑面试官可约时间 → 对待约面候选人发起排期 → Agent 取时间交集、分配最早时段
